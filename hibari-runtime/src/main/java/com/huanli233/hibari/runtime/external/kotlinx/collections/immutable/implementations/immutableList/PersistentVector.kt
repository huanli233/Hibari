...
internal class PersistentVector<E>(private val root: Array<Any?>,
    ...
    init {
        ...
        assert(size - rootSize(
            size
        ) <= tail.size.coerceAtMost(MAX_BUFFER_SIZE))
    }
    ...
    private fun pushTail(root: Array<Any?>?, shift: Int, tail: Array<Any?>): Array<Any?> {
        val bufferIndex = indexSegment(
            size - 1,
            shift
        ) // size - 1 is the index of the last element in the new trie
    ...
    private fun insertIntoRoot(root: Array<Any?>, shift: Int, index: Int, element: Any?, elementCarry: ObjectRef): Array<Any?> {
        ...
        if (shift == 0) {
            val newRoot = if (bufferIndex == 0) arrayOfNulls<Any?>(MAX_BUFFER_SIZE) else root.copyOf(
                MAX_BUFFER_SIZE
            )
    ...
    override fun listIterator(index: Int): ListIterator<E> {
        ...
        return PersistentVectorIterator(
            ...
            index,
            size,
            rootShift / LOG_MAX_BUFFER_SIZE + 1
        )
    }
