...
package com.huanli233.hibari.runtime.external.kotlinx.collections.immutable.implementations.immutableMap

import com.huanli233.hibari.runtime.external.kotlinx.collections.immutable.PersistentMap
import com.huanli233.hibari.runtime.external.kotlinx.collections.immutable.internal.DeltaCounter
...
internal open class PersistentHashMapBuilder<K, V>(private var map: PersistentHashMap<K, V>) : PersistentMap.Builder<K, V>, AbstractMutableMap<K, V>() {
    ...
    override fun remove(key: K): V? {
        ...
        node = node.mutableRemove(key.hashCode(), key, 0, this) ?: TrieNode.Companion.EMPTY as TrieNode<K, V>
        return operationResult
    }
    ...
    fun remove(key: K, value: V): Boolean {
        val oldSize = size
        @Suppress("UNCHECKED_CAST")
        node = node.mutableRemove(key.hashCode(), key, value, 0, this) ?: TrieNode.Companion.EMPTY as TrieNode<K, V>
        return oldSize != size
    }

    override fun clear() {
        @Suppress("UNCHECKED_CAST")
        node = TrieNode.Companion.EMPTY as TrieNode<K, V>
        size = 0
    }
}