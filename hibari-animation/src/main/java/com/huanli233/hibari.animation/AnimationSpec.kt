...
import com.huanli233.hibari.animation.ArcMode.Companion.ArcBelow
import com.huanli233.hibari.animation.ArcMode.Companion.ArcLinear
...
value class StartOffsetType private constructor(internal val value: Int) {
    companion object {
        ...
        val Delay: StartOffsetType = StartOffsetType(-1)

        /** Fast forwards the animation to a given play time, and starts it immediately. */
        val FastForward: StartOffsetType = StartOffsetType(1)
    }
}
 ...
 */
class KeyframesSpec<T>(val config: KeyframesSpecConfig<T>) :
    DurationBasedAnimationSpec<T> {
    /**
 ...
 */ fun <T> snap(delayMillis: Int = 0): SnapSpec<T> = SnapSpec<T>(delayMillis)
